...
const BookingCards2 = ({ onSelect, selectedItems, id,setectedTimes,setDateId}) => {
    ...
    <div className="p-4 mb-4">
      ...
      {uniqueDates.map((date, index) => (
        <div
          ...
          onClick={() => handleSelectItem('Item X', date, dateData.find(item => item.date === date).times, dateData.find(item => item.date === date).id)}
        >
          ...
          <div key={data.id}>
            ...
            <ul>
              {data.times.map((time, index) => (
                <li onClick={() => handleSubmit(data.id,time)} key={index}>
                  <div className='flex'> <input type="radio" className='p-4' name="sessionType"  /><p className='text-red-900 font-bold'>{time}</p></div>
                </li>
              ))}
            </ul>
          </div>
